...
rs1 = 0.1; % reflectivity of backscatterer 1 - was 0.5
...
% 
 for indexi=1:rows(resizedim) 
  ...
  bscantransposedhilb(indexi,:)=abs(ifft(plinearbth(indexi,:).*exp(i*angl(indexi,:))));
...
% bscang(1,:)=zeros(1,rows(resizedim));
 ...
 figure;
 imagesc(10*log10(bscanhilb(1:160,:)./max(bscanhilb(1:160,:)))); title('Hilb method, BTH Window');colorbar;
 ...
 plot(xaxis,10*log10(bscanhilb(1:160,230)./max(bscanhilb(1:160,230))),'.');
 hold off;
