...
% Chapter 9
% and the Hilbert method in
% http://journal.iisc.ernet.in/index.php/iisc/article/view/1113
%           
%            Hilbert( abs(1+log(f(w))) ) = angle(1+f(w))
%            ifft[(1+f(w))exp(j angle(1+f(w))] = delta(0) + f(z)
%
% Without windowing and apodization
%
% Hari Nandakumar
%
% $Date: 02 Nov 2018
...
rs1 = 0.05; % reflectivity of backscatterer 1 - was 0.5
rs2 = 0.05; %rs2 = 0.0; % reflectivity of backscatterer 2 was 0.25
...
%apodi   = resizedim./resizedbk;       % apodi = (1+f(w))^2
apodi = resizedim;
logf = log(sqrt(apodi));
angl = imag(hilbert(abs(logf)));
...
%  bscantransposedbth(indexi,:)=abs(ifft(plinearbth(indexi,:)));
  bscantransposedhilb(indexi,:)=abs(ifft(plinear(indexi,:).*exp(i.*angl(indexi,:))));
...
% imagesc(10*log10(bscanbth(1:160,:)./max(bscanbth(1:160,:)))); title('BTH Window');colorbar;
 figure;
 imagesc(10*log10(bscanhilb(1:160,:)./max(bscanhilb(1:160,:)))); title('Hilb method');colorbar;
...
% plot(bscang(1:160,230)); title('Linear plot, Gaussian window');
 ...
 plot(xaxis,10*log10(bscan(1:160,230)./max(bscan(1:160,230))),'r'); xlabel('microns');hold on;
% plot(xaxis,10*log10(bscant(1:160,230)./max(bscant(1:160,230))),'k');
 plot(xaxis,10*log10(bscanhilb(1:160,230)./max(bscanhilb(1:160,230))),'b');
% hold off;
 
% figure;
% plot(real(angl(2,:))); title('Real part of h');
% figure;
% plot(imag(angl(2,:))); title('Im part of h');
% figure;
% plot(abs(angl(2,:))); title('Absolute value of h');
% figure;
% plot(arg(angl(2,:))); title('Angle of h');