...
#endif
#ifdef _WIN64
//#include <qhyccd.h> webcam version wo need for QHY
#endif


/*
* modified from BscanFFT.cpp
* to demonstrate the software with webcam support
* without needing QHY camera.
*
*
...
* *************************************
* EXPOSURE settings are not used in WEBCAM VERSION
* Also, bit-depth defaults to 8 in WEBCAM VERSION
* *************************************
...
// and for webcam,
// https://docs.opencv.org/3.4/d8/dfe/classcv_1_1VideoCapture.html#details
...
int main(int argc, char *argv[])
{
	...
	// webcam demo version
	std::cout<<"WEBCAM demo version. Exposure and other camera controls are dummy."<<std::endl;
	std::cout<<" A key toggles averaging"<<std::endl;
	std::cout<<" Q key toggles clamping upper bound of displayed Bscan to 50 dB"<<std::endl;
	std::cout<<" ] key increases thresholding in final Bscan"<<std::endl;
	std::cout<<" [ key decreases thresholding in final Bscan"<<std::endl;
	std::cout<<" 9 or ( key decreases the index of the reported ascan max value"<<std::endl;
	std::cout<<" 0 or ) key increases the index of the reported ascan max value "<<std::endl;
	std::cout<<" "<<std::endl;
	std::cout<<" w decreases width of ROI for which avg val is reported, W increases width"<<std::endl;
	std::cout<<" h decreases the height (position), H increases"<<std::endl;
	std::cout<<" location of ROI is to the right of the index of reported ascan"<<std::endl;
	std::cout<<" e toggles rEporting/plotting of ROI average intensity"<<std::endl;
	std::cout<<" "<<std::endl;
	std::cout<<" ESC, x or X key quits"<<std::endl;
	/////////////////////
	int num = 0;
	...
	//char camtype[16];
	int found = 0;
	unsigned int w, h, bpp = 8, channels, cambitdepth = 8;
...
/* commenting out QHY specific code
...
*/
		...
		ret = 1; //SetQHYCCDParam(camhandle, CONTROL_EXPOSURE, camtime); //handle, parameter name, exposure time (which is in us)
		if (ret == 1)
		{
		...
		{
			ret = 1; //GetQHYCCDLiveFrame(camhandle, &w, &h, &bpp, &channels, mraw.data);
			cap.read(frame);
